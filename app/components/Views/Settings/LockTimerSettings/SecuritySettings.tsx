...
import { setLockTime, setIdleLock } from '../../../../actions/settings';
...
const Settings: React.FC = () => {
  ...
  const selectLockTime = (time: string): void => {
    ...
    dispatch(setIdleLock(idleLockEnabled));
  };

  const idleLock = useSelector(
    // TODO: Replace "any" with type
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (state: any) => state.settings.idleLock,
  );

  // const toggleIdleLock = useCallback(
  //   (value: boolean) => {
  //     dispatch(setAllowLoginWithRememberMe(value));
  //   },
  //   [dispatch],
  // );
  // const [isUsingIdleLock, setIsUsingIdleLock] = useState<boolean>(false);
  // const onValueChanged = useCallback(
  //   (enabled: boolean) => {
  //     isUsingIdleLock
  //       ? toggleIdleLock(enabled)
  //       : '';
  //   },
  //   [isUsingIdleLock, navigation.navigate, toggleIdleLock],
  // );

  const triggerClose = (): void => navigation.goBack();
  const [selectTime, setSelectTime] = useState<string>(
    lockTime.toString(),
  );
  const [idleLockEnabled, setIdleLockEnabled] = useState<boolean>(idleLock);
  return (
    <ScrollView
    ...
    >
      <View style={styles.setting} testID={AUTO_LOCK_SECTION}>
        <Text
          ...
          style={styles.desc}
        >
          {strings('settings.auto_lock_desc')}
        </Text>
            ...
            <View style={styles.listWrapper}>
              {AUTO_LOCK_OPTIONS.map((option) => (
                <TouchableOpacity
                  ...
                  onPress={() => setSelectTime(option.value)}
        ...
        <SecurityOptionToggle
          title={strings(`settings.idle_lock`)}
          description={strings(`settings.idle_lock_desc`)}
          value={idleLockEnabled}
          onOptionUpdated={(value) => setIdleLockEnabled(value)}
          testId={'turn-on-idle-lock'}
        />
        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          <Button
            ...
            width={ButtonWidthTypes.Full}
            testID={'lock-timer-cancel-button'}
          />
          <Button
            style={{ flex: 1 }}
            variant={ButtonVariants.Primary}
            size={ButtonSize.Lg}
            onPress={() => selectLockTime(selectTime)}
            label={strings('settings.idle_lock_save')}
            width={ButtonWidthTypes.Full}
            isDanger
            testID={'lock-timer-save-button'}
          />
        </View>
      </View>
